      ...
      int size, bool& parseSuccess, bool& calculateDimensionSuccess, char** errorMessage)
	{
      ...
      try
      {
         ...
         for(int i=0;i< size;i++)
         {
            ...
            DimensionInfo dimensionInfo(dimInfoStruct.Length, dimInfoStruct.Mass, dimInfoStruct.Time,
               dimInfoStruct.ElectricCurrent, dimInfoStruct.Temperature, dimInfoStruct.Amount, dimInfoStruct.LuminousIntensity);

            quantityDimensions.push_back(QuantityDimensionInfo(dimInfoStruct.QuantityName, dimensionInfo));
         }
